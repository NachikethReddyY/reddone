...
async function executeRollback(workspaceId: string, runId: string, fencingToken: WorkflowFencingToken) {
  "use step";
  const db = getDb();
...
async function executeRelease(workspaceId: string, runId: string, fencingToken: WorkflowFencingToken) {
  "use step";
  const db = getDb();
...
async function failRun(workspaceId: string, runId: string, fencingToken: WorkflowFencingToken, unsafeMessage: string) {
  "use step";
  const db = getDb();
...
async function claimRun(workspaceId: string, runId: string) {
  "use step";
  const db = getDb();
...
async function executeResearch(workspaceId: string, runId: string, fencingToken: WorkflowFencingToken) {
  "use step";
  const db = getDb();
  const run = await db.workflowRun.findFirstOrThrow({
    where: { id: runId, workspaceId },
...
async function executeSelectedFindingSpecification(
  workspaceId: string,
  runId: string,
  fencingToken: WorkflowFencingToken,
...
async function executeBuild(workspaceId: string, runId: string, fencingToken: WorkflowFencingToken, polish: boolean) {
  "use step";
  const db = getDb();
  const run = await db.workflowRun.findFirstOrThrow({
    where: { id: runId, workspaceId },
