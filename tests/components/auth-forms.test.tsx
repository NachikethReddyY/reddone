...
describe("authentication forms", () => {
  it("propagates a validated return path through username sign in", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith("/api/auth/sign-in/username", expect.anything());
    expect(JSON.parse(String(init.body))).toMatchObject({ username: "owner", callbackURL: "/account?section=sessions" });
    await waitFor(() => expect(replace).toHaveBeenCalledWith("/account?section=sessions"));
  });

  it("lets returning hackathon participants sign in with username and password", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/^Username/), { target: { value: "owner" } });
    fireEvent.change(screen.getByLabelText("Password"), { target: { value: "correct horse battery staple" } });
    fireEvent.click(screen.getByRole("button", { name: "Sign in with username" }));

    await waitFor(() => expect(fetch).toHaveBeenCalledOnce());
    expect(fetch).toHaveBeenCalledWith("/api/auth/sign-in/username", expect.objectContaining({
      method: "POST",
      body: JSON.stringify({
        username: "owner",
  ...
  it("keeps email sign in as a compatibility path for existing owners", async () => {
    vi.mocked(fetch).mockResolvedValue(new Response(JSON.stringify({ url: "/projects" }), { status: 200, headers: { "content-type": "application/json" } }));
    render(<SignInForm deploymentMode="private" returnTo="/projects" />);

    fireEvent.change(screen.getByLabelText(/^Username/), { target: { value: "OWNER@EXAMPLE.COM" } });
    fireEvent.change(screen.getByLabelText("Password"), { target: { value: "correct horse battery staple" } });
    fireEvent.click(screen.getByRole("button", { name: "Enter control plane" }));

    await waitFor(() => expect(fetch).toHaveBeenCalledWith("/api/auth/sign-in/email", expect.objectContaining({
      body: JSON.stringify({
  ...
  it("marks required signup fields and associates invalid input with stable feedback", async () => {
    ...
    for (const input of [name, username, email, password, confirmation]) expect(input).toBeRequired();

    fireEvent.change(name, { target: { value: "N" } });
    fireEvent.change(username, { target: { value: "nora" } });
  ...
  it("creates an invite-gated owner and signs in with the return path", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/^Username/), { target: { value: "Nora.Dev" } });
    ...
    expect(fetch).toHaveBeenNthCalledWith(2, "/api/auth/sign-in/username", expect.objectContaining({
      body: JSON.stringify({
        username: "nora.dev",
        password: "long public password",
        rememberMe: true,
  ...
  it("returns to beta access when the invite session is unavailable", async () => {
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, "/api/owner/register", expect.objectContaining({
      method: "POST",
      body: JSON.stringify({ name: "Nora", username: "nora", email: "nora@example.com", password: "long public password" }),
    }));
